...
sudo apt install acpi audacious alsa-utils arandr build-essential curl dkms eject extrepo feh ftp git gpodder innoextract libdrm-dev linux-headers-$(uname -r) mc mpv mupdf mutt ncal network-manager newsboat p7zip-full pavucontrol pulseaudio qbittorrent rxvt-unicode smartmontools tmux tlp timeshift udiskie unar unrar-free vim w3m wget x11-xserver-utils xclip -y
