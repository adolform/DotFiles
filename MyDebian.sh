...
sudo apt install acpi audacious alsa-utils arandr build-essential cmatrix cowsay curl dkms eject extrepo feh firefox-esr ftp git imagemagick innoextract libdrm-dev lightdm linux-headers-$(uname -r) mc mpv ncal network-manager p7zip-full papirus-icon-theme pavucontrol picom pulseaudio qbittorrent rxvt-unicode sioyek smartmontools tmux tlp timeshift udiskie unar unrar-free unattended-upgrades vim wget x11-xserver-utils xclip xcowsay -y
